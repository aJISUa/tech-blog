...
함수 이름을 안 쓴다. `요리하기`처럼 이름을 붙이지 않고 괄호만 쓰고, 가운데 화살표 `->`가 들어간다. 처음 보면 좀 이상하게 생겼다. 수학의 람다 대수(λ-calculus)에서 온 이름이고, 파이썬의 `lambda`랑 같은 뿌리다.
...
원래라면 `Hello h = new Hello...` 이런 식으로 써야 할 것 같은데, 화살표 식 하나만 붙였는데도 돌아간다. 메소드를 구현하려면 객체가 있어야 하는데, 람다식이 알아서 객체를 만들고 메소드를 구현해서 `h`가 가리키게 해준다.

`x -> x * x`에서 `x`가 int인지 double인지 정하지도 않았다. 변수 타입인 `Square`에 추상 메소드가 `calc(int x)` 하나뿐이라서, 컴파일러가 그걸 보고 `x`를 int로 알아서 정해준다.
...
메소드 이름(`sayHello`, `calc`, `operate`)은 별로 중요하지 않다. 교수님께서 일부러 다 다르게 지었다고 하셨다.
...
한두 줄짜리에 로직도 별로 없는 코드는 이렇게 쓰는 게 훨씬 깔끔하다.
