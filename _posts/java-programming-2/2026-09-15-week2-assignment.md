...
tags:
  ...
  - 싱글톤
  - 과제
excerpt: "교수님이 주신 배틀 코드 3개와 Vehicle-Driver 코드에서 숨은 문제를 재현하고 고쳐봤다. 짱구네 집 다형성이랑 하늘 싱글톤도 직접 돌려서 확인했다."
toc: true
toc_sticky: true
---
...
- 리팩토링: 수업 퀴즈였던 Vehicle-Driver 코드를 먼저 연습 삼아 고쳐보고, 교수님이 리팩토링하라고 주신 배틀 코드 3개를 같은 방식(문제 재현, 수정, 전후 비교)으로 고쳤다. 배틀 쪽은 길어서 따로 글로 뺐다
...
교수님이 리팩토링하라고 주신 배틀 코드 3개(삼국지, 해리포터, 몬스터 배틀)는 내용이 길어서 따로 정리했다.
...
- **엄마처럼, 할머니처럼 요리하기:** 한 단계 위로 올라가니까 각각 "스파게티 요리하기", "된장과 고추장을 만들기"
- **Quiz #4:**
...
한글 클래스 이름이라 실행이 될지 조금 걱정했는데, 이름만 맞춰주니까 잘 돌아간다. 프로젝트 인코딩이 UTF-8이라 한글 파일 이름이랑 클래스 이름을 문제없이 읽는다.
