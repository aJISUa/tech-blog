---
title: "[자프실2] 2주차 과제: Vehicle-Driver, 배틀 리팩토링, 짱구네 집, 하늘 싱글톤"
...
tags:
  ...
  - 싱글톤
  - 과제
excerpt: "교수님께서 주신 배틀 코드 3개와 Vehicle-Driver 코드에서 숨은 문제를 재현하고 고쳐봤다. 짱구네 집 다형성이랑 하늘 싱글톤도 직접 돌려서 확인했다."
toc: true
toc_sticky: true
---
...
- 리팩토링: 수업 퀴즈였던 Vehicle-Driver 코드를 먼저 연습 삼아 고쳐보고, 교수님께서 리팩토링하라고 주신 배틀 코드 3개를 같은 방식(문제 재현, 수정, 전후 비교)으로 고쳤다. 배틀 쪽은 길어서 따로 글로 뺐다
...
교수님께서 배틀 코드 보여주실 때 "실행은 너무 잘 되는데 열어보면 심각한 문제가 있다"고 하셨던 게 생각나서, 결과가 맞게 나오는 이 코드도 한번 의심해보기로 했다. 코드를 읽으면서 이상한 부분을 적어두고, 진짜 문제가 되는지 테스트 코드를 따로 만들어서 확인했다.
...
BugDemo 맨 아래에는 퀴즈 6번 답이었던 코드도 주석으로 넣어뒀다.

```java
Moveable m = new 버스("타요버스", 100, 12);
((Driveable) d).drive(m);          // d는 위에서 만든 미미
```

#### 실행 전에 예상한 것

코드를 읽으면서 생각한 대로라면 이렇게 나와야 했다.

- **문제1:** 탱크A는 연료 5L에 10km마다 2.5L를 쓰니까 딱 20km 가고 멈춘다. 두 번째 drive는 연료가 0이라 한 칸도 못 간다. 그런데 move()가 이번에 간 거리가 아니라 누적 거리를 돌려주니까 20이 한 번 더 더해져서, 드라이버 기록은 40km가 될 것 같다.
- **문제2:** 탱크B는 3L라서 10km 가면 0.5L가 남는다. 멈추는 조건이 `연료량 <= 0.01`이라 0.5L로도 한 번 더 출발할 테니, 20km까지 가고 연료는 -2.00이 될 것 같다.
- **문제3:** 빈 생성자로 만든 버스는 이름이 null, 연비가 0.0이다. `10/연비`는 0으로 나누기인데, double끼리 나누는 거라 에러 대신 Infinity가 나올 것 같다.
- **문제4:** Driveable에는 `drive()`밖에 없으니까, Driveable로 바라본 d에서는 `drive(m)`이 안 보여서 컴파일 에러가 날 것 같다.
...
코드를 읽으면서 어렴풋이 짐작은 했지만 확신은 없었는데, 결과는 예상한 대로 전부 나왔다. 특히 문제3은 0으로 나누면 에러가 날지 Infinity가 나올지 헷갈렸다. double끼리 나누면 에러 없이 Infinity가 나온다. 탱크A는 20km만 갔는데 드라이버 기록은 40km고, 탱크B는 연료가 -2.00까지 내려갔고, 빈 생성자로 만든 차는 이름이 null에 Infinity가 찍혔다. 퀴즈에 나온 값(100, 100, 50)은 연료가 딱 나누어떨어져서 원래 Main에서는 이런 문제가 안 보였던 거였다.

문제4는 주석을 풀자마자 `drive`에 빨간 줄이 생겼고, 그래도 실행하니까 Driveable의 `drive()`는 Moveable을 인자로 받을 수 없다는 에러가 났다. 짱구 퀴즈에서 배운 "어떤 타입으로 보느냐에 따라 보이는 게 다르다"가 인터페이스에서도 그대로 나와서 좀 신기했다.
...
`move()`에 final을 붙여서 하위 클래스가 공통 동작을 건드리지 못하게 했다. `소리내기()`는 abstract라서 새 차를 추가할 때 이걸 안 만들면 컴파일 에러가 난다. 버스는 `운행하기()`, 비행기는 `fly()`, 탱크는 println을 바로 쓰던 제각각인 방식도 `소리내기()` 하나로 맞춰졌다.
...
원래 Main 시나리오를 그대로 돌려보고, BugDemo에서 찾은 문제가 고쳐졌는지는 RefactorCheck라는 코드를 따로 만들어서 확인했다.

#### 실행 전에 예상한 것

- **원래 Main:** 연료를 빼고 주행거리를 더하는 계산은 그대로 두고 멈추는 조건이랑 리턴값만 고쳤으니까, 표에 찍히는 숫자(1200, 1210, 1410)는 원본이랑 똑같아야 한다. 대신 운전할 때 문구는 "미미이(가) 버스을(를) 운전합니다."처럼 바뀐다.
- **확인1:** 이제 move()가 이번에 간 거리만 돌려주니까, 연료가 없는 두 번째 drive에서는 0이 더해져서 드라이버 기록도 20km일 것 같다.
- **확인2:** 3L에서 10km 가면 0.5L가 남고, 2.5L가 안 되니까 더는 출발하지 않아서 0.50L가 남을 것 같다.
- **확인3:** 연비가 0이면 생성자에서 예외를 던지니까 "생성 실패" 메시지가 나올 것 같다.
- **확인4:** Driveable에 `drive(Moveable m)`을 추가했으니 컴파일이 되고, 앞에서 탱크A로 20km를 운전한 같은 미미가 비행기로 10km를 더 가서 총 주행거리는 30이 될 것 같다.

#### 실제 결과

![리팩토링 후 Vehicle-Driver 실행 결과]({{ '/assets/images/java-programming-2/vd-refactor-console.png' | relative_url }})

리팩토링한 프로젝트에서 같은 Main을 돌린 결과다. 예상대로 운전 문구만 "미미이(가) 비행기을(를) 운전합니다."로 바뀌었고, 주행거리와 연료는 원본이랑 똑같다. 두 출력을 diff로 비교해봐도 표 숫자는 한 줄도 안 달랐다.

![리팩토링 후 Vehicle-Driver 클래스 다이어그램]({{ '/assets/images/java-programming-2/vd-refactor-uml.png' | relative_url }})

원본 다이어그램이랑 비교해보면 차이가 바로 보인다. Driveable에 `drive(m: Moveable)`, `drive(ms: Moveable[])`가 추가됐고, Driver는 Scanner 없이 drive 메소드가 2개로 줄었다. Vehicle은 빈 생성자와 setter가 사라지고 `MOVE_UNIT_KM`, `EPSILON` 상수가 생겼다. 버스, 비행기는 생성자랑 `소리내기()`만 남았고, 탱크는 여기에 `포탄쏘기()`가 더 있다.

![RefactorCheck 실행 결과]({{ '/assets/images/java-programming-2/vd-refactor-check-console.png' | relative_url }})

RefactorCheck도 예상한 대로 나왔다. 같은 탱크를 두 번 운전해도 20km로 기록되고, 3L 탱크는 10km만 가고 0.50L가 남고, 연비 0인 차는 만들어지지 않고, 퀴즈 6번 코드도 컴파일돼서 총 주행거리 30까지 잘 나온다. 확인4는 비행기만 운전했으니 10인지 잠깐 헷갈렸는데, 앞에서 탱크A로 20km를 운전한 같은 미미라서 30이 맞다.
...
| Driver의 drive 메소드 | 5개 (+ 인터페이스 default 1개) | 2개 (+ 인터페이스 default 1개) |
...
교수님께서 리팩토링하라고 주신 배틀 코드 3개(삼국지, 해리포터, 몬스터 배틀)는 내용이 길어서 따로 정리했다.

[배틀 리팩토링 글 보기]({{ site.baseurl }}{% post_url java-programming-2/2026-09-15-battle-refactoring %})
...
- **Quiz #1** `엄마 mom = new 짱구(); mom.요리하기()`: 엄마 타입이라 스파게티가 먼저 떠오르지만, 실제로 만든 건 짱구니까 "컵라면 먹기"가 나올 것 같다.
- **Quiz #2** `a`, `b`, `c`: 전부 짱구 객체라 요리하기는 셋 다 "컵라면 먹기", 청소하기는 "더럽히기"일 것 같다. `a.청소하기()`는 할머니 설계도에 없어서 컴파일 에러가 날 테니 주석으로 막아뒀다.
- **Quiz #3** 엄마처럼, 할머니처럼 요리하기: 한 단계 위로 올라가니까 각각 "스파게티 요리하기", "된장과 고추장을 만들기"
- **Quiz #4:**
  - 핸드폰이 있을 때는 "공부하는 척 ..."이 5번 나온다.
  - 엄마가 압수하면 짱구 폰은 null이 되고 엄마 bag에 폰이 들어가서 "열심히 공부중..."
  - 짱구가 되찾으면 짱구 폰과 아까 엄마 bag에 있던 폰의 주소가 같고, 엄마 bag은 null이 된다. 폰이 다시 생겼으니 또 공부하는 척만 한다.

#### 실제 결과

![다형성테스트.java 실행 결과]({{ '/assets/images/java-programming-2/jjanggu-polymorphism-console.png' | relative_url }})

헷갈리긴 했지만 결과는 예상한 대로 나왔다. 특히 Quiz #1은 엄마 타입으로 부르니까 스파게티일 것 같은 느낌이 계속 들었는데, `mom`은 엄마 타입인데도 컵라면이 나왔다. 부르는 건 엄마 타입으로 불렀지만 실행된 건 짱구 메소드다.

`할머니 a`로 `a.청소하기()`를 쓰면 컴파일 에러가 난다. 할머니 설계도에는 청소하기가 없으니까.

핸드폰을 되찾은 뒤에 짱구 폰이랑 아까 엄마 bag에 있던 폰의 해시코드가 @7e6cbb7a로 같게 나왔다. 해시코드는 다른 객체끼리 우연히 같을 수도 있으니까, 확실히 하려면 싱글톤에서 했던 것처럼 `==`로 비교해보는 게 맞다. 엄마 bag은 null이 됐다. 옮긴 다음에 원래 자리를 비워야 핸드폰이 두 개가 되는 일이 없다.
...
- 생성자가 private이라 `getInstance()`로만 객체를 얻을 수 있고, 처음 부를 때만 새로 만드니까 "하늘 객체를 만들어요"는 한 번만 찍힐 것 같다.
- sky1, sky2, sky3는 전부 같은 객체라서 @ 뒤 해시코드가 셋 다 같고, `==` 비교는 true일 것 같다.

#### 실제 결과

![하늘 싱글톤 실행 결과]({{ '/assets/images/java-programming-2/singleton-console.png' | relative_url }})

한글 클래스 이름이라 실행이 될지 조금 걱정했는데, 이름만 맞춰주니까 잘 돌아간다. 소스 파일 인코딩이 UTF-8이라 한글 클래스 이름도 문제없이 읽는다. 파일 이름은 인코딩이 아니라 윈도우가 알아서 처리해주는 부분이다.
