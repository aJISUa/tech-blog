...
tags:
  ...
  - 상속
  - 과제
excerpt: "교수님께서 리팩토링하라고 주신 지난 학기 배틀 코드 3개에서 숨은 문제를 테스트 코드로 재현하고, 원본은 그대로 둔 채 새 프로젝트에서 고쳤다."
toc: true
toc_sticky: true
---
...
교수님께서 리팩토링하라고 주신 코드는 지난 학기 자프실1 배틀 과제 코드 세 개였다. 폴더 이름에 힌트가 붙어 있었다.
...
`Player`에 이름, hp, power, mp, 이미지 두 개가 있는데 관우, 유비, 장비, 조조가 이 여섯 개를 전부 또 선언하고 있었다. 게다가 전용무기 대신 캐릭터마다 청룡, 쌍고 같은 자기 무기 필드를 따로 들고 있었다. 관우 코드는 이렇다.
...
- 관우, 유비, 장비, 조조가 `Player`에 있는 필드 6개를 똑같이 다시 선언했다. 수업에서 교수님께서 PlantUML로 보여주신 "Player에 있는데 유비에서 또 선언한" 그 문제다.
...
- 관우에는 `private static final String String = null;`이라는 쓸데없는 필드가 있다.
...
두 캐릭터 이미지가 다 보이고, Harry에 넣은 값 그대로 hp 500, power 30으로 시작한다. 공격한 쪽 버튼은 비활성화돼서 번갈아 누르게 된다.
...
예상한 대로 전부 고쳐졌다. 값이 그대로 들어가고, 캐릭터마다 다른 이미지를 갖고, 무기를 들면 데미지가 늘어난다.
...
몇 턴 진행한 화면이다. 턴마다 기록 창에 공격, 연주, 치료 결과가 쌓인다. 이 캡처에는 창 위쪽 캐릭터 그림이 안 보이는데, 아래에서 고칠 문제들과는 상관없는 부분이다.
...
원본과 같은 배치로, 같은 순서대로 턴이 돌아간다. 캐릭터 코드를 화면에서 떼어냈지만 기록 창 문구는 거의 그대로 나온다. 원본에서 붙어 있던 "공격이Vestal에게" 같은 부분에 띄어쓰기가 들어간 정도만 다르다.
...
- 1번 `Battle`처럼 규칙을 화면에서 떼어내는 게 교수님께서 말씀하신 "상속을 회피하는 방법(연관 관계)"이랑 같은 이야기일까?
