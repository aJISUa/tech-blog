...
| 폴더 | 게임 | 힌트 | 코드 |
| --- | --- | --- | --- |
| 1_상속 | 삼국지 배틀 (RTKBattle) | 상속을 잘못 썼다 | 13개 파일, 746줄 |
| 2_상속 | 해리포터 배틀 (OOP_Battle) | 상속을 1번과 다른 방식으로 잘못 썼다 | 12개 파일, 471줄 |
| 3_모듈화_비교문 | 몬스터 배틀 (battle) | 모듈화가 안 돼 있고 비교문이 많다 | 13개 파일, 1254줄 |
...
- `Win`은 그리기와 버튼 연결만 한다. 똑같이 생긴 진행바, 초상화, 버튼 만드는 코드는 작은 메소드로 묶었다. 342줄이던 `Win`이 125줄이 됐다.
...
광대는 `roll(10) + 1`, 해골 궁정인은 `5 + roll(6)`, 성녀는 `roll(6)` 한 줄씩이다. 광대 클래스는 104줄에서 34줄이 됐다.
