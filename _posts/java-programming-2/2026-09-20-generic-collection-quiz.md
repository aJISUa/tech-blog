...
2번 질문의 답: Set의 "중복"은 내가 생각하는 중복이 아니라 `hashCode()`와 `equals()`가 판단하는 중복이다. 슬라이드에 "HashSet은 hashcode로 동등 객체 여부를 판단한다", "다른 기준을 쓸 경우 hashCode()와 equals()를 재정의해 동등 객체가 될 조건을 정해야 한다"고 나온 게 이거다. 내가 만든 클래스를 Set에 넣거나 Map의 키로 쓸 거면 둘 다 오버라이딩해야 한다.
